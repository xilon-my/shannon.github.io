...
- **embedding 模型 = transformer + 池化层**。池化把"一串 token 向量"并成"一个向量"。一句话进来:先过 transformer,得到每个 token 的上下文向量;再池化成一个向量——这就是"这句话的嵌入"。判断两句话相不相似,就是比较这两个向量。
- **reranker = transformer + 打分头**。把 query 和 passage 拼进**同一个序列**,过 transformer,再拿整串的某种表示过一个打分头,输出一个相关性分数。因为 query 和 passage 在同一个序列里,注意力会让它们逐 token 互看。

所以"双塔"和"单塔"的区别,本质是 transformer 的注意力作用在什么上:query 和 passage 是**分开编码**(注意力只在各自内部跑,最后比两个向量),还是**拼在一起编码**(注意力跨两段文本互通,最后直接打分)。这一个差别,决定了下面所有关于速度、精度、训练的讨论。先看双塔。

### 双塔:把判断提前到入库

embedding 模型的"双塔"是字面意思:同一个 transformer 复制成两份,共享同一套权重,一个塔编码查询、一个塔编码文档,各自输出一个向量,最后比这两个向量(余弦,和前一篇一样)。为什么长成这个形状?这有个极好的动机故事。2019 年之前,"两句话相不相似"用的是原生 BERT——把两句话拼成一个序列丢进去。它很准,但贵到离谱:在一万句话里找最相似的一对,需要跑约 5000 万次两两拼接的推理,约 65 小时。SBERT 的解法(Sentence-BERT,2019,arXiv:1908.10084)就是双塔:两个 BERT 共享权重,各自把一句话编码成一个向量,再比余弦。离线把一万句话全部编码一遍只要约 5 秒,查询时和一个新向量比相似度只要 0.01 秒。

![SBERT 孪生双塔架构](/discover/sbert-arch-8bit.png)
*图:SBERT 用共享权重的两个 BERT 把句子各自编码成向量(arXiv:1908.10084,Figure 1)*

先解一个最容易绕晕的点:embedding 模型本身**不比较任何东西**,它只做一件事——把一个文本变成一个向量。"相似度"是编码完之后、在模型外面算的:拿两个向量算余弦。那为什么叫"双塔"、图上要画两个?因为检索这个场景天然涉及**两个**文本:一个查询、一个文档。你得用同一个模型把它们各编成一个向量,它们才落在同一个向量空间里、才比得了。图上的两座塔其实是同一个 transformer 画了两遍——左边编码查询、右边编码文档,权重是同一套。这也反过来解释了为什么权重必须共享:查询里的"退款政策"和文档里的"退款政策",如果进了两个不同的模型,就会落在两个不同的空间,永远比不了。

代价是把判断提前了:入库时就得决定"这句话是什么",查询时只剩几何比对。注意权重是共享的——两个塔如果各学一套权重,同一个词就会得到两套向量,query 和 passage 就比不起来了。为了让这个提前的判断更准,双塔用了一个巧劲——决定怎么把"一串 token 向量"并成"一个向量"(池化)。比如一句话 20 个 token,过 transformer 得到 20 个 768 维的上下文向量,mean pooling 就是把这 20 个向量按维度取平均,并成一个 768 维的句子向量;取 [CLS] 是直接拿第 1 个位置([CLS] 是 BERT 在句首放的一个分类符)的那个向量;取最后一个 token 是 LLM 底座的惯例。三种做法里,池化不是随便选的——它和损失、数据一起决定向量质量。骨干从 BERT 类编码器走向解码器 LLM 后,有两个适配坑:causal mask 掩码只让每个 token 看左边,限制整段编码([NV-Embed](https://github.com/NVIDIA/NV-Embed) 改成双向注意力 + latent pooling);EOS-last 池化有 recency bias(left-padding 时按 attention mask 取位置,不能无脑取最后一个)。
...
reranker 用单塔(cross-encoder):把 query 和 passage 拼成**同一个序列**([CLS] query [SEP] passage [SEP],[SEP] 是 BERT 用来隔开两段文本的分隔符),过同一个 transformer。关键在注意力:因为是一个序列,query 的每个 token 可以直接 attend 到 passage 的每个 token——"逐 token 互看"是字面意思,两段文本的信息在每一层注意力里互通,不需要先各自压缩。编码完之后,拿整个序列的某种表示(通常还是 [CLS] 位置的向量,或再做一次池化)过一个打分头,输出一个相关性分数。这里没有"压成一颗向量"这一步——query 的词可以直接和 passage 的词对上,这就是它准的原因。打分头有两种姿势:分类头(bge-reranker-v2-m3,过一层线性层输出两类 logits,取两类差值当分数;logits 是模型最后一层输出的未归一化原始分数)和生成头(MonoT5,让模型接着生成 "true" 或 "false" 这个词,取两个词的 logit 差)。

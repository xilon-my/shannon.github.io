...
表里的数字是"典型报道区间,具体取决于语料与训练数据"——怎么用你自己的数据验证,第五节再讲。用一个真实样例感受"看得多细"的差别:query 是"退款政策",文档里有一句"如果您的订单还未发货,可以申请退款;关于退货的流程见下面的细则"。双塔把整句平均池化成一颗向量后判 0.72,单塔逐 token 互看判 0.91。差距从哪来?"退款""政策"这几个精确词被揉进均值向量里,和一大堆泛泛的词混在一起,信号被稀释成"平均意义上像退款";而逐 token 交互保留了硬匹配——虽然原文写的是"申请退款"和"退货的流程",但问题里的"退款"能直接和句子里的"退款"对齐,不需要先把整句话压成一个点再比。这就是"看得多细"的落点:双塔牺牲精确词换速度,单塔牺牲速度换精确词。也是"检索为什么要分两段"的由来:双塔先粗筛(快但粗),单塔再精排(准但贵)。
...
先澄清两个容易混的点。一是这些 token 向量从哪来:不是"分块"切出来的,也不是另一个 embedding 模型给的——就是 ColBERT 自己那个 transformer 编码器,对每个 token 输出的上下文向量。它不做 RAG 那种语义分块,整段文本过一遍,每个 token 留一个向量。二是它和 embedding、reranker 的训练关系:三者共用同一族损失——喂 (query, 相关文档, 不相关文档),让相关文档的得分高于不相关的;区别只在打分函数:embedding 用单向量点积,reranker 用拼起来的 cross-attention,ColBERT 用 token 级 MaxSim。这一层关系,第三节开头会正式讲透。

![ColBERT 架构](/discover/colbert-arch-8bit.png)
*图:ColBERT 的晚期交互,查询和文档的 token 向量逐对 MaxSim(arXiv:2004.12832,Figure 3)*

它不是第三种模型,是曲线上的中点。这个"中点"很实在:一套输出,两个角色都能当——token 向量可以像双塔一样预计算、建索引,当**检索器**用(全库捞 top-k);MaxSim 打分又可以像单塔一样对 top-k 逐对精排,当 **reranker** 用。注意它没有池化层、不产单向量,所以"粗筛"不是把文档 pool 成单向量再比,而是把 MaxSim 拆成两级:入库时先对文档的 token 向量做聚类(聚的是 token,不是 chunk;ColBERTv2 的 PLAID 索引),查询时每个 query token 先找到最近的簇、锁定候选文档集(近似,粗),再对候选逐对算精确 MaxSim 排序(精)。所以这个中点比想象中常用——Cohere 的商业 rerank 就以晚交互为主力;bge-m3 的"三合一"里也有一个 ColBERT 引擎,第六节会回来引用。
...
在 3a、3b 之前,先点破一个全局:embedding、reranker、ColBERT 的训练信号本质是同一个——让相关的 (query, 文档) 得分高于不相关的。区别只在两处:打分函数(单向量点积 / cross-attention / MaxSim),和损失的形态。"对比学习"这个词通常指 embedding 那边的 InfoNCE 或三元组;reranker 那边更多叫"排序学习"(learning to rank),其中 pairwise 和 listwise 也是对比思想,pointwise 则是纯分类。下面会看到,它们共用同一个底子。

### 双塔:把"语义"训进向量空间

第一节讲过双塔训练的目标——正对拉近、负对推远。这一节把它讲透:这个目标背后有两个力在同时作用——alignment(正对尽量近)和 uniformity(所有向量在超球面上尽量均匀、别塌缩成一团)。对比学习同时优化这两个力,这也是它能修掉 BERT 句向量"各向异性"问题的原因:无关句子算出来余弦平均也有约 0.42,mean pooling 还会把 token 级的 0.446 放大到句子级的 0.924——向量都挤在同一个锥里,没法区分。对比学习把这个锥旋转开。
...
第一节讲过单塔学排序、数据分 pointwise/pairwise/listwise 三档——这一节看具体怎么训。三分框架出自 Tie-Yan Liu 2009 年的专著,排序损失家族二十年没换骨架:RankNet(2005,成对概率 + 交叉熵,曾支撑 Bing)→ LambdaRank(2006,按 ΔNDCG 加权)→ LambdaMART(2010,赢了 Yahoo! Challenge)。落到今天的 cross-encoder:bge-reranker 家族用 pointwise 两类交叉熵;Qwen3-Reranker 用 pointwise(生成式 yes/no 的 BCE)加 listwise——一个 query 配 1 个正样本和 n 个负样本,对 n+1 个分数做带温度的 softmax,目标就是正样本排第一。listwise 更贴合真实检索的形态:检索时你面对的从来不是"这一个相关、那一个不相关"的二分类,而是一整排候选要排出先后。

...
但我一直在想一个问题：**一定要提前定义 Agent 角色吗？**

Squad 的模型还是"先定义谁负责什么，再分配任务"——前端 Agent、后端 Agent、设计 Agent，每个有固定的 system prompt 和上下文。这跟公司的组织架构一样，好处是清晰，坏处是僵化。

另一种思路是：只有一个 runtime，不预设任何角色。主 Agent 接到任务后自己判断怎么拆，需要的时候动态召子 Agent，传当前阶段需要的上下文，用完就丢。子 Agent 没有固定的身份，它们是函数式的——调一个子 Agent 就像调一个函数，输入参数，拿回结果。

这个模型的好处是不需要人来做角色设计。你不需要提前想好"我需要几个 Agent，各自干什么"，主 Agent 根据任务动态决定怎么拆、要不要召子 Agent。

我说完发现这不就是 Symphony 的思路吗——背后一个 runtime，WORKFLOW.md 定义行为，Agent 按状态流转工作。只不过我对 WORKFLOW.md 的要求更高：不只是写一段 prompt 给 Agent 读，而是能定义"什么情况下召子 Agent"、"子 Agent 传什么上下文"、"状态怎么流转"这些规则。现在的 WORKFLOW.md 更像个 prompt 模板，我想要的是在 WORKFLOW.md 里定义一套**判断逻辑**——声明式的、Agent 自己读自己执行的那种。

两种思路没有绝对对错。Squad 适合团队分工明确的场景（前端就是前端、后端就是后端），单 runtime + 增强版 WORKFLOW.md 适合任务边界模糊、需要灵活应变的场景。

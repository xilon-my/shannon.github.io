...
这个模型的好处是不需要人来做角色设计。你不需要提前想好"我需要几个 Agent，各自干什么"，主 Agent 根据任务动态决定怎么拆、要不要召子 Agent。结构是活的，不是硬编码的。

关键问题在于怎么定义"什么时候召子 Agent"的判断规则。一个自然的想法是写在 WORKFLOW.md 里——声明式的规则，Agent 自己读自己执行。但这又回到"到底怎么描述这个判断逻辑"的问题上，我现在还没想清楚最好的写法。

两种思路没有绝对对错。Squad 适合团队分工明确的场景（前端就是前端、后端就是后端），单 runtime 模型适合任务边界模糊、需要灵活应变的场景。
